...
def manage_cards(btn1, btn2, btn3,country,version,platform,start_date,end_date):
    ...
    if "btn-nclicks-1" == ctx.triggered_id:
        ...
        if start_date and not country and not version and not platform:
            mask = ((date_string_to_date(posDf["Date"]) >= start_date) & (date_string_to_date(posDf["Date"]) <= end_date))
            data = posDf.loc[mask]
        if start_date and country and version and platform:
            mask = (((date_string_to_date(posDf["Date"]) >= start_date) & (date_string_to_date(posDf["Date"]) <= end_date)) & posDf['Country'].isin(country) & posDf['Version'].isin(version) & posDf['Platform'].isin(platform))
            data = posDf.loc[mask]
        if start_date and country and not version and not platform:
            mask = ((date_string_to_date(posDf["Date"]) >= start_date) & (date_string_to_date(posDf["Date"]) <= end_date)& posDf['Country'].isin(country))
            data = posDf.loc[mask]
        if start_date and not country and version and not platform:
            mask = ((date_string_to_date(posDf["Date"]) >= start_date) & (date_string_to_date(posDf["Date"]) <= end_date)& posDf['Version'].isin(version))
            data = posDf.loc[mask]
        if start_date and not country and not version and platform:
            mask = ((date_string_to_date(posDf["Date"]) >= start_date) & (date_string_to_date(posDf["Date"]) <= end_date)& posDf['Platform'].isin(platform))
            data = posDf.loc[mask]
        if start_date and country and version and not platform:
            mask = ((date_string_to_date(posDf["Date"]) >= start_date) & (date_string_to_date(posDf["Date"]) <= end_date)& posDf['Country'].isin(country)& posDf['Version'].isin(version))
            data = posDf.loc[mask]
        if start_date and country and not version and platform:
            mask = ((date_string_to_date(posDf["Date"]) >= start_date) & (date_string_to_date(posDf["Date"]) <= end_date)& posDf['Country'].isin(country)& posDf['Platform'].isin(platform))
            data = posDf.loc[mask]
        if start_date and not country and version and platform:
            mask = ((date_string_to_date(posDf["Date"]) >= start_date) & (date_string_to_date(posDf["Date"]) <= end_date)& posDf['Version'].isin(version)& posDf['Platform'].isin(platform))
            data = posDf.loc[mask]
        ...
        if not start_date:
            data = mixDf
        if start_date and not country and not version and not platform:
            mask = ((date_string_to_date(mixDf["Date"]) >= start_date) & (date_string_to_date(mixDf["Date"]) <= end_date))
            data = mixDf.loc[mask]
        if start_date and country and version and platform:
            mask = (((date_string_to_date(mixDf["Date"]) >= start_date) & (date_string_to_date(mixDf["Date"]) <= end_date)) & mixDf['Country'].isin(country) & mixDf['Version'].isin(version) & mixDf['Platform'].isin(platform))
            data = mixDf.loc[mask]
        if start_date and country and not version and not platform:
            mask = ((date_string_to_date(mixDf["Date"]) >= start_date) & (date_string_to_date(mixDf["Date"]) <= end_date)& mixDf['Country'].isin(country))
            data = mixDf.loc[mask]
        if start_date and not country and version and not platform:
            mask = ((date_string_to_date(mixDf["Date"]) >= start_date) & (date_string_to_date(mixDf["Date"]) <= end_date)& mixDf['Version'].isin(version))
            data = mixDf.loc[mask]
        if start_date and not country and not version and platform:
            mask = ((date_string_to_date(mixDf["Date"]) >= start_date) & (date_string_to_date(mixDf["Date"]) <= end_date)& mixDf['Platform'].isin(platform))
            data = mixDf.loc[mask]
        if start_date and country and version and not platform:
            mask = ((date_string_to_date(mixDf["Date"]) >= start_date) & (date_string_to_date(mixDf["Date"]) <= end_date)& mixDf['Country'].isin(country)& mixDf['Version'].isin(version))
            data = mixDf.loc[mask]
        if start_date and country and not version and platform:
            mask = ((date_string_to_date(mixDf["Date"]) >= start_date) & (date_string_to_date(mixDf["Date"]) <= end_date)& mixDf['Country'].isin(country)& mixDf['Platform'].isin(platform))
            data = mixDf.loc[mask]
        if start_date and not country and version and platform:
            mask = ((date_string_to_date(mixDf["Date"]) >= start_date) & (date_string_to_date(mixDf["Date"]) <= end_date)& mixDf['Version'].isin(version)& mixDf['Platform'].isin(platform))
            data = mixDf.loc[mask]
        for index, row in data.iterrows():
            cusName = row["Name"]
            review = row["Review"]
        ...
        if not start_date:
            data = negDf
        if start_date and not country and not version and not platform:
            mask = ((date_string_to_date(negDf["Date"]) >= start_date) & (date_string_to_date(negDf["Date"]) <= end_date))
            data = negDf.loc[mask]
        if start_date and country and version and platform:
            mask = (((date_string_to_date(negDf["Date"]) >= start_date) & (date_string_to_date(negDf["Date"]) <= end_date)) & negDf['Country'].isin(country) & negDf['Version'].isin(version) & negDf['Platform'].isin(platform))
            data = negDf.loc[mask]
        if start_date and country and not version and not platform:
            mask = ((date_string_to_date(negDf["Date"]) >= start_date) & (date_string_to_date(negDf["Date"]) <= end_date)& negDf['Country'].isin(country))
            data = negDf.loc[mask]
        if start_date and not country and version and not platform:
            mask = ((date_string_to_date(negDf["Date"]) >= start_date) & (date_string_to_date(negDf["Date"]) <= end_date)& negDf['Version'].isin(version))
            data = negDf.loc[mask]
        if start_date and not country and not version and platform:
            mask = ((date_string_to_date(negDf["Date"]) >= start_date) & (date_string_to_date(negDf["Date"]) <= end_date)& negDf['Platform'].isin(platform))
            data = negDf.loc[mask]
        if start_date and country and version and not platform:
            mask = ((date_string_to_date(negDf["Date"]) >= start_date) & (date_string_to_date(negDf["Date"]) <= end_date)& negDf['Country'].isin(country)& negDf['Version'].isin(version))
            data = negDf.loc[mask]
        if start_date and country and not version and platform:
            mask = ((date_string_to_date(negDf["Date"]) >= start_date) & (date_string_to_date(negDf["Date"]) <= end_date)& negDf['Country'].isin(country)& negDf['Platform'].isin(platform))
            data = negDf.loc[mask]
        if start_date and not country and version and platform:
            mask = ((date_string_to_date(negDf["Date"]) >= start_date) & (date_string_to_date(negDf["Date"]) <= end_date)& negDf['Version'].isin(version)& negDf['Platform'].isin(platform))
            data = negDf.loc[mask]
        for index, row in data.iterrows():
            cusName = row["Name"]
            review = row["Review"]
